...
from experiment.prompts_factorial_abd import generate_factorial_prompt_cases_abd
from experiment.providers import AnthropicProvider, GeminiProvider, OpenAIProvider, ProviderError
from experiment.parser_abd import parse_choice_abd
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run factorial framing experiment with choices restricted to A/B/D (no C)"
    )
    p.add_argument(
        "--providers",
...
def _ratios_abd(counts: dict[str, int]) -> dict[str, float]:
    denom = sum(counts.get(k, 0) for k in ("A", "B", "D"))
    if denom <= 0:
        return {k: 0.0 for k in ("A", "B", "D")}
    return {k: 100.0 * counts.get(k, 0) / denom for k in ("A", "B", "D")}
...
def main() -> int:
    ...
    cases = generate_factorial_prompt_cases_abd(n=args.n, seed=args.seed)
    ...
    for provider_name, provider in providers.items():
        ...
        tmp_path = os.path.join(out_dir, f"factorial_abd_{provider_name}_{ts}.txt")
        writer = ExperimentWriter(out_path=tmp_path)
        writer.write_header(
            title="LLM Factorial Framing Experiment (A/B/D only)",
            meta={
        ...
        counts = {"A": 0, "B": 0, "D": 0, "UNKNOWN": 0, "ERROR": 0, "TOTAL": 0}
        ...
        for i, case in enumerate(cases, start=1):
            ...
            try:
                ...
                choice = parse_choice_abd(text)

                if choice not in ("A", "B", "D"):
                    counts["UNKNOWN"] += 1
        ...
        ratios = _ratios_abd(counts)

        writer._f.write("=" * 80 + "\n")
        writer._f.write("SUMMARY\n\n")
        writer._f.write(f"Provider: {provider_name}\n")
        writer._f.write(f"Model: {model_name}\n")
        writer._f.write(f"A count: {counts['A']} (ratio among A/B/D: {ratios['A']:.2f}%)\n")
        writer._f.write(f"B count: {counts['B']} (ratio among A/B/D: {ratios['B']:.2f}%)\n")
        writer._f.write(f"D count: {counts['D']} (ratio among A/B/D: {ratios['D']:.2f}%)\n")
        ...
        for ck in sorted(cond_counts.keys()):
            ...
            writer._f.write(
                f"  A: {c['A']} ({r['A']:.2f}%)  B: {c['B']} ({r['B']:.2f}%)  D: {c['D']} ({r['D']:.2f}%)\n"
            )
            writer._f.write(
        ...
        final_name = f"{_sanitize(model_name)}_{ratios['A']:.2f}_{ratios['B']:.2f}_{ratios['D']:.2f}_{ts}.txt"
